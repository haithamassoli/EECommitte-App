import { MapperItem } from "@Types/index";
import { screenWidth } from "@Utils/Helper";

const MAPPING: MapperItem[] = [
  {
    id: 1,
    shape: "circle",
    x1: (screenWidth / 100) * 62,
    y1: ((screenWidth * 1.074) / 100) * 13,
    radius: 25,
  },
  {
    id: 2,
    shape: "circle",
    x1: (screenWidth / 100) * 62,
    y1: ((screenWidth * 1.074) / 100) * 20,
    radius: 25,
  },
  {
    id: 3,
    shape: "circle",
    x1: (screenWidth / 100) * 58,
    y1: ((screenWidth * 1.074) / 100) * 27,
    radius: 25,
  },
  {
    id: 4,
    shape: "circle",
    x1: (screenWidth / 100) * 67,
    y1: ((screenWidth * 1.074) / 100) * 26,
    radius: 25,
  },
  {
    id: 5,
    shape: "circle",
    x1: (screenWidth / 100) * 72,
    y1: ((screenWidth * 1.074) / 100) * 33,
    radius: 25,
  },
  {
    id: 6,
    shape: "circle",
    x1: (screenWidth / 100) * 45,
    y1: ((screenWidth * 1.074) / 100) * 13,
    radius: 25,
  },
  {
    id: 7,
    shape: "circle",
    x1: (screenWidth / 100) * 45,
    y1: ((screenWidth * 1.074) / 100) * 20,
    radius: 25,
  },
  {
    id: 8,
    shape: "circle",
    x1: (screenWidth / 100) * 46,
    y1: ((screenWidth * 1.074) / 100) * 27,
    radius: 20,
  },
  {
    id: 9,
    shape: "circle",
    x1: (screenWidth / 100) * 36,
    y1: ((screenWidth * 1.074) / 100) * 20,
    radius: 25,
  },
  {
    id: 10,
    shape: "circle",
    x1: (screenWidth / 100) * 36,
    y1: ((screenWidth * 1.074) / 100) * 13,
    radius: 25,
  },
  {
    id: 11,
    shape: "circle",
    x1: (screenWidth / 100) * 35,
    y1: ((screenWidth * 1.074) / 100) * 29,
    radius: 25,
  },
  {
    id: 12,
    shape: "circle",
    x1: (screenWidth / 100) * 27,
    y1: ((screenWidth * 1.074) / 100) * 15,
    radius: 25,
  },
  {
    id: 13,
    shape: "circle",
    x1: (screenWidth / 100) * 15,
    y1: ((screenWidth * 1.074) / 100) * 23,
    radius: 25,
  },
  {
    id: 14,
    shape: "circle",
    x1: (screenWidth / 100) * 16,
    y1: ((screenWidth * 1.074) / 100) * 32,
    radius: 30,
  },
  {
    id: 15,
    shape: "circle",
    x1: (screenWidth / 100) * 6,
    y1: ((screenWidth * 1.074) / 100) * 33,
    radius: 25,
  },
  {
    id: 16,
    shape: "circle",
    x1: (screenWidth / 100) * 53,
    y1: ((screenWidth * 1.074) / 100) * 21,
    radius: 25,
  },
  {
    id: 17,
    shape: "circle",
    x1: (screenWidth / 100) * 71,
    y1: ((screenWidth * 1.074) / 100) * 16,
    radius: 25,
  },
  {
    id: 18,
    shape: "circle",
    x1: (screenWidth / 100) * 88,
    y1: ((screenWidth * 1.074) / 100) * 24,
    radius: 25,
  },
  {
    id: 19,
    shape: "circle",
    x1: (screenWidth / 100) * 88,
    y1: ((screenWidth * 1.074) / 100) * 31,
    radius: 25,
  },
  {
    id: 20,
    shape: "circle",
    x1: (screenWidth / 100) * 40,
    y1: ((screenWidth * 1.074) / 100) * 34,
    radius: 25,
  },
  {
    id: 21,
    shape: "circle",
    x1: (screenWidth / 100) * 58,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 22,
    shape: "circle",
    x1: (screenWidth / 100) * 41,
    y1: ((screenWidth * 1.074) / 100) * 42,
    radius: 25,
  },
  {
    id: 23,
    shape: "circle",
    x1: (screenWidth / 100) * 52,
    y1: ((screenWidth * 1.074) / 100) * 34,
    radius: 25,
  },
  {
    id: 24,
    shape: "circle",
    x1: (screenWidth / 100) * 52,
    y1: ((screenWidth * 1.074) / 100) * 42,
    radius: 25,
  },
  {
    id: 25,
    shape: "circle",
    x1: (screenWidth / 100) * 28,
    y1: ((screenWidth * 1.074) / 100) * 34,
    radius: 25,
  },
  {
    id: 26,
    shape: "circle",
    x1: (screenWidth / 100) * 28,
    y1: ((screenWidth * 1.074) / 100) * 42,
    radius: 25,
  },
  {
    id: 27,
    shape: "circle",
    x1: (screenWidth / 100) * 67,
    y1: ((screenWidth * 1.074) / 100) * 42,
    radius: 25,
  },
  {
    id: 28,
    shape: "circle",
    x1: (screenWidth / 100) * 68,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 29,
    shape: "circle",
    x1: (screenWidth / 100) * 88,
    y1: ((screenWidth * 1.074) / 100) * 38,
    radius: 25,
  },
  {
    id: 30,
    shape: "circle",
    x1: (screenWidth / 100) * 88,
    y1: ((screenWidth * 1.074) / 100) * 46,
    radius: 25,
  },
  {
    id: 31,
    shape: "circle",
    x1: (screenWidth / 100) * 80,
    y1: ((screenWidth * 1.074) / 100) * 46,
    radius: 25,
  },
  {
    id: 32,
    shape: "circle",
    x1: (screenWidth / 100) * 15,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 33,
    shape: "circle",
    x1: (screenWidth / 100) * 5,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 34,
    shape: "circle",
    x1: (screenWidth / 100) * 5,
    y1: ((screenWidth * 1.074) / 100) * 58,
    radius: 25,
  },
  {
    id: 35,
    shape: "circle",
    x1: (screenWidth / 100) * 28,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 36,
    shape: "circle",
    x1: (screenWidth / 100) * 39,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 37,
    shape: "circle",
    x1: (screenWidth / 100) * 49,
    y1: ((screenWidth * 1.074) / 100) * 50,
    radius: 25,
  },
  {
    id: 38,
    shape: "circle",
    x1: (screenWidth / 100) * 45,
    y1: ((screenWidth * 1.074) / 100) * 56,
    radius: 25,
  },
  {
    id: 39,
    shape: "circle",
    x1: (screenWidth / 100) * 80,
    y1: ((screenWidth * 1.074) / 100) * 24,
    radius: 25,
  },
  {
    id: 40,
    shape: "circle",
    x1: (screenWidth / 100) * 19,
    y1: ((screenWidth * 1.074) / 100) * 74,
    radius: 30,
  },
  {
    id: 42,
    shape: "circle",
    x1: (screenWidth / 100) * 32,
    y1: ((screenWidth * 1.074) / 100) * 67,
    radius: 25,
  },
  {
    id: 43,
    shape: "circle",
    x1: (screenWidth / 100) * 40,
    y1: ((screenWidth * 1.074) / 100) * 64,
    radius: 25,
  },
  {
    id: 44,
    shape: "circle",
    x1: (screenWidth / 100) * 49,
    y1: ((screenWidth * 1.074) / 100) * 63,
    radius: 25,
  },
  {
    id: 45,
    shape: "circle",
    x1: (screenWidth / 100) * 32,
    y1: ((screenWidth * 1.074) / 100) * 75,
    radius: 25,
  },
  {
    id: 46,
    shape: "circle",
    x1: (screenWidth / 100) * 32,
    y1: ((screenWidth * 1.074) / 100) * 82,
    radius: 25,
  },
  {
    id: 47,
    shape: "circle",
    x1: (screenWidth / 100) * 42,
    y1: ((screenWidth * 1.074) / 100) * 74,
    radius: 25,
  },
  {
    id: 48,
    shape: "circle",
    x1: (screenWidth / 100) * 42,
    y1: ((screenWidth * 1.074) / 100) * 81,
    radius: 25,
  },
  {
    id: 49,
    shape: "circle",
    x1: (screenWidth / 100) * 53,
    y1: ((screenWidth * 1.074) / 100) * 73,
    radius: 25,
  },
  {
    id: 50,
    shape: "circle",
    x1: (screenWidth / 100) * 17,
    y1: ((screenWidth * 1.074) / 100) * 63,
    radius: 25,
  },
  {
    id: 51,
    shape: "circle",
    x1: (screenWidth / 100) * 58,
    y1: ((screenWidth * 1.074) / 100) * 67,
    radius: 25,
  },
  {
    id: 52,
    shape: "circle",
    x1: (screenWidth / 100) * 57,
    y1: ((screenWidth * 1.074) / 100) * 58,
    radius: 30,
  },
  {
    id: 53,
    shape: "circle",
    x1: (screenWidth / 100) * 76,
    y1: ((screenWidth * 1.074) / 100) * 65,
    radius: 25,
  },
  {
    id: 54,
    shape: "circle",
    x1: (screenWidth / 100) * 75,
    y1: ((screenWidth * 1.074) / 100) * 73,
    radius: 25,
  },
  {
    id: 55,
    shape: "circle",
    x1: (screenWidth / 100) * 66,
    y1: ((screenWidth * 1.074) / 100) * 67,
    radius: 25,
  },
  {
    id: 56,
    shape: "circle",
    x1: (screenWidth / 100) * 84,
    y1: ((screenWidth * 1.074) / 100) * 69,
    radius: 30,
  },
  {
    id: 57,
    shape: "circle",
    x1: (screenWidth / 100) * 88,
    y1: ((screenWidth * 1.074) / 100) * 82,
    radius: 25,
  },
  {
    id: 58,
    shape: "circle",
    x1: (screenWidth / 100) * 58,
    y1: ((screenWidth * 1.074) / 100) * 82,
    radius: 25,
  },
  {
    id: 59,
    shape: "circle",
    x1: (screenWidth / 100) * 71,
    y1: ((screenWidth * 1.074) / 100) * 82,
    radius: 25,
  },
  {
    id: 60,
    shape: "circle",
    x1: (screenWidth / 100) * 27,
    y1: ((screenWidth * 1.074) / 100) * 59,
    radius: 30,
  },
  {
    id: 61,
    shape: "circle",
    x1: (screenWidth / 100) * 78,
    y1: ((screenWidth * 1.074) / 100) * 82,
    radius: 30,
  },
  {
    id: 62,
    shape: "circle",
    x1: (screenWidth / 100) * 81,
    y1: ((screenWidth * 1.074) / 100) * 54,
    radius: 25,
  },
  {
    id: 63,
    shape: "circle",
    x1: (screenWidth / 100) * 86.5,
    y1: ((screenWidth * 1.074) / 100) * 60,
    radius: 25,
  },
  {
    id: 64,
    shape: "circle",
    x1: (screenWidth / 100) * 8,
    y1: ((screenWidth * 1.074) / 100) * 74,
    radius: 30,
  },
  {
    id: 1000,
    shape: "circle",
    x1: (screenWidth / 100) * 88,
    y1: ((screenWidth * 1.074) / 100) * 16,
    radius: 25,
  },
  {
    id: 1001,
    shape: "circle",
    x1: (screenWidth / 100) * 80,
    y1: ((screenWidth * 1.074) / 100) * 16,
    radius: 25,
  },
  {
    id: 1002,
    shape: "circle",
    x1: (screenWidth / 100) * 80,
    y1: ((screenWidth * 1.074) / 100) * 31,
    radius: 25,
  },
  {
    id: 1003,
    shape: "circle",
    x1: (screenWidth / 100) * 80,
    y1: ((screenWidth * 1.074) / 100) * 38,
    radius: 25,
  },
  {
    id: 1004,
    shape: "circle",
    x1: (screenWidth / 100) * 29.5,
    y1: ((screenWidth * 1.074) / 100) * 24,
    radius: 20,
  },
  {
    id: 1005,
    shape: "circle",
    x1: (screenWidth / 100) * 22,
    y1: ((screenWidth * 1.074) / 100) * 24,
    radius: 20,
  },
];
export default MAPPING;
