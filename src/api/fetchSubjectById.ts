...
export function fetchSubjectById(id: number, refetchCounter: number) {
  const { data, isLoading, isFetching } = useQuery(
    ...
    async () => {
      const lastRequest = getDataMMKV(`lastRequestsubjectById${id}`);
      ...
      if (
        ...
        storeDataMMKV(`lastRequestsubjectById${id}`, cacheExpiryTime);
        const snapshot = querySnapshot.docs.map((doc) => {
          return { ...doc.data(), uid: doc.id };
        })[0];
        storeDataMMKV(`subjectById${id}`, snapshot);
        return snapshot;
      } else {
        const subject = getDataMMKV(`subjectById${id}`);
        if (subject == null) {
          return [];
        }
        return subject;
